export default class LocalStore {
  constructor(storeName, existingStore, errorCallback) {
    ...
    if (existingStore) {
      try {
        sessionStorage[storeName] = existingStore;
      }
      catch(error) {
        if (error) {
          errorCallback(error);
  ...
  getAll() {
    return JSON.parse(
      sessionStorage[this.storeName]
    );
  }

  setAll(mutatedStore) {
    sessionStorage[this.storeName] = JSON.stringify(mutatedStore);
  }
  ...
  reset() {
    sessionStorage[this.storeName] = JSON.stringify([]);
  }
}
